...
class Transitioner extends Highway.Transition {
  ...
  _fade(from, to, done) {
    this._scrollToTop();
    var t1 = new TimelineLite();
    t1.to(from, { duration: 0.1, opacity: "0" }).fromTo(
      to,
      { opacity: "0" },
      {
        duration: 0.1,
  ...
  in({ from, to, done, trigger }) {
    ...
    if (
      trigger === "popstate" ||
      (!trigger.classList.contains("hamburger-link") &&
        !trigger.classList.contains("logo-link"))
    ) {
      ...
      const isToPortfolio = toName === "portfolio";

      //create booleans regarding the from name
      const isFromHome = fromName === "home";
      const isFromAbout = fromName === "about";
      const isFromPortfolio = fromName === "portfolio";

      //determine the animation based on the to and from pages
      if (
        (isFromHome && isToPortfolio) ||
        (isFromPortfolio && isToAbout) ||
        (isFromAbout && isToHome)
      ) {
        //right
        this._slideRight(from, to, done);
      } else if (
        (isFromHome && isToAbout) ||
        (isFromAbout && isToPortfolio) ||
        (isFromPortfolio && isToHome)
      ) {
        //left
      ...
      //If the transition was triggered by anything in the nav bar or an unknown source, default to the crossfade
      this._fade(from, to, done);
    }
  }
